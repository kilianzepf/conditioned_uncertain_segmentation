...
from models.ssn import StochasticUnet
...
def train(
    ...
    for epoch in tqdm(range(resume_epoch, epochs)):  # may be error in range
        ...
        model.eval()
        with torch.no_grad():
            for images, masks, seg_dist, _ in val_loader:
                counter += 1
                # Send tensors to cuda
                images = images.to(device)
                masks = masks.to(device)
...
if __name__ == "__main__":
    ...
    config = dict(
        ...
        loss="See Paper",
        architecture="Stochastic Segmentation Network",
    ...
    if args.testit == True:
        ...
        if W:
            wandb.init(
                ...
                group="SSN Unets",
                job_type="Testing",
                notes=f"Testing model {training_run_name}",
                config=config,
                dir="/scratch/kmze",
            )
        ...
        evaluate_ssn.evaluate(
            model=ssn,
            test_loader=test_loader,
            treshold_for_testing=0.5,
            metadata=meta,
